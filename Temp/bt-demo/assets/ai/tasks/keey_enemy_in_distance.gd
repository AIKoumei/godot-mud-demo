...
func _generate_name() -> String:
	return "keep enemy in a distance"
...
func _tick(_delta: float) -> Status:
	...
	var dir_v2 = agent.global_position - target.global_position
	dir_v2 = dir_v2.normalized()
	var range_min = 200
	var range_max = 400
	var flank_pos = target.global_position + dir_v2 * (range_max - range_min)
	...
	if agent.global_position.distance_to(flank_pos) < TOLERANCE:
		return SUCCESS
	...
	var desired_velocity: Vector2 = agent.global_position.direction_to(flank_pos) * speed * _delta
