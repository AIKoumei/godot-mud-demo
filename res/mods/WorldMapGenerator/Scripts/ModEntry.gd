...
func _gen_all_locations_map():
	...
	after_gen_all_locations_map_finished.call_deferred()
	
	is_thread_running = false
	map_gen_thread.wait_to_finish()
...
func after_gen_all_locations_map_finished():
	...
	emit_mod_event.call_deferred("after_gen_all_locations_map_finished", {
...
func gen_all_locations_map():
	...
	if GameCore.debugging and SaveManager.has_mod_slot_file(GameCore.Settings.GameSettings.GameSlot, "%s/total_location_map_datas.sav" % [mod_name]):
		...
		cur_location_map_data = total_location_map_datas[total_location_map_datas.keys()[0]]
	...
	map_gen_thread.start(_gen_all_locations_map)
