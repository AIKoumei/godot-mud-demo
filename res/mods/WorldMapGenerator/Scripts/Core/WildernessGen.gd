...
static func simplex_noise(width, height, seed_offset):
    ...
    noise.octaves = 2              ## 噪声八度，控制细节
    noise.period = CONFIG["noise_scale"]  ## 噪声周期，控制缩放
    noise.persistence = 0.5        ## 噪声持久性，控制频率影响
...
static func generate_color_map(input_data):
    ...
    for y in range(height):
        for x in range(width):
            ...
            var r = int(hex_color.substr(1, 2), 16)
            var g = int(hex_color.substr(3, 2), 16)
            var b = int(hex_color.substr(5, 2), 16)
...
static func generate_map(seed, output_dir):
    ## 确保输出目录存在
    ...
    ## 保存 JSON 文件
    var json_filename = output_dir + "/seed_" + str(seed) + ".final_data.json"
    ...
    ## 保存颜色地图
    var color_map_filename = output_dir + "/5.1.seed." + str(seed) + ".color.png"
    ## 注意：Godot 中保存图片需要使用 Image 类
    ## 这里简化处理，实际项目中可能需要更复杂的实现
