...
class_name WorldMapGeneratorTownGen
extends RefCounted
...
class MapGenerator:
	...
	func step_1_generate_mask() -> Dictionary:
		...
		for point_str in original_mask:
			var x, y = point_str.split(",").map(func(s): return int(s))
		...
		while check_queue.size() > 0:
			...
			var x, y = edge_point.split(",").map(func(s): return int(s))
		...
		if start_node:
			...
			while queue.size() > 0:
				...
				var x, y = current_node.split(",").map(func(s): return int(s))
	...
	func step_3_generate_roads(center_data: Dictionary) -> Dictionary:
		...
		var center_x, center_y = center.split(",").map(func(s): return int(s))
		var primary_road = []
		var gates = []
		var gate_walls = []
			...
			while true:
				...
				if current_point in edges:
					...
					if is_last_contour:
						...
						var gate_x, gate_y = current_point.split(",").map(func(s): return int(s))
						# 计算垂直方向的偏移
						if dx != 0: # 水平方向的主干道（左右）
							# 垂直方向（上下）生成城门城墙
							for dy_off in [-1, 1]:
	...
	func step_5_generate_blocks(wall_data: Dictionary) -> Dictionary:
		...
		# 检查点是否有效（不在障碍物中）
		func is_valid_point(x: int, y: int) -> bool:
				...
				for block_size in [5, 4, 3, 2, 1]:
					# 计算区块的四个角坐标
						...
						if block_valid:
							...
							for road_point in primary_road:
								var road_x, road_y = road_point.split(",").map(func(s): return int(s))
								# 计算区块中心到主干道的距离
								var block_center_x = x + block_size // 2
								var block_center_y = y + block_size // 2
		...
		for road_point in secondary_roads:
			if road_point not in visited:
				...
				while queue.size() > 0:
					...
					var x, y = current.split(",").map(func(s): return int(s))
					var directions = [[0, -1], [0, 1], [-1, 0], [1, 0]] # 上、下、左、右
		...
		for component in connected_components:
			...
			for road_point in component:
				var x, y = road_point.split(",").map(func(s): return int(s))
				var directions = [[0, -1], [0, 1], [-1, 0], [1, 0]] # 上、下、左、右
